...
export default function AuditDetailPage() {
  ...
  const isOwner = audit.officerId === user.userId;
  const isAdmin = user.role === 'CITY_ADMINISTRATOR';
  const canUpdate = (isOwner || isAdmin) && audit.status !== 'CLOSED';

  // Whether the audit is in a terminal "done" state — if so the active step
  // should also render as completed (checkmark + green) so the entire
  // timeline reads as finished.
  const isTerminal = audit.status === 'CLOSED';
    ...
    <>
      ...
      <div className="card">
        ...
        <div className="steps">
          {statusSteps.map((step, i) => {
            const completed = i < currentIdx || (i === currentIdx && isTerminal);
            const active    = i === currentIdx && !isTerminal;
            return (
              <div key={step} className={`step ${completed ? 'completed' : active ? 'active' : ''}`}>
                <div className="step-dot">{completed ? '✓' : i + 1}</div>
      ...
      <div className="card">
        ...
        <div className="profile-grid">
          ...
          <div className="profile-item"><label>Officer</label><div className="value">{audit.officerName} (ID: #{audit.officerId})</div></div>
        ...
        <div style={{ marginTop: 24, padding: 20, background: 'var(--gray-50)', borderRadius: 14, border: '1px solid var(--gray-100)' }}>
          <label style={{ fontSize: '.75rem', fontWeight: 800, color: 'var(--gray-400)', textTransform: 'uppercase', letterSpacing: 1, display: 'block', marginBottom: 8 }}>Scope
          </label>
        ...
        {audit.findings && (
          <div style={{ marginTop: 16, padding: 20, background: '#eff6ff', borderRadius: 14, border: '1px solid #bfdbfe' }}>
            <label style={{ fontSize: '.75rem', fontWeight: 800, color: 'var(--gray-400)', textTransform: 'uppercase', letterSpacing: 1, display: 'block', marginBottom: 8 }}>Findings
            </label>
        ...
        <div className="actions-row">
          ...
          {canUpdate && nextStatus && (
            <button className="btn btn-primary" onClick={() => { setUpdateForm({ status: nextStatus, findings: audit.findings || '' }); setShowUpdate(true); }}>Move to {nextStatus.replace('_', ' ')}
            </button>
          )}
        </div>
      </div>
      ...
      {showUpdate && (
        <div className="modal-overlay" onClick={() => setShowUpdate(false)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            <h3>Update Audit to {updateForm.status.replace('_', ' ')}</h3>
            <form onSubmit={handleUpdate}>
              ...
              <div className="actions-row">
                <button className="btn btn-primary" disabled={updating}>{updating ? 'Updating...' : `Confirm ${updateForm.status.replace('_', ' ')}`}</button>
