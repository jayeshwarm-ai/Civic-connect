...
export default function ResolutionDetailPage() {
  ...
  return (
    <>
      {/* Resolution Info */}
      <div className="card">
        <div className="card-title">Resolution #{resolution.resolutionId}
        ...
        <div className="actions-row">
          ...
          {canManage && <button className="btn btn-primary" onClick={() => setShowAddStep(true)}>Add Step</button>}
        </div>
      </div>
            ...
            <form onSubmit={handleAddStep}>
              <div className="form-group">
                ...
                <textarea
                  ...
                  rows={3}
                  placeholder="Describe the task or action step..."
                  required
                />
              </div>
              <div className="actions-row">
                <button className="btn btn-primary" disabled={addingStep}>
                  {addingStep ? 'Adding...' : 'Add Step'}
                </button>
                <button type="button" className="btn btn-outline" onClick={() => setShowAddStep(false)}>Cancel
                </button>
              </div>
            </form>
          </div>
      ...
      {/* Steps */}
      <div className="card">
        <div className="card-title">Steps ({steps.length})</div>
        {steps.length === 0 ? (
                    ...
                    <tr key={s.stepId}>
                      ...
                      <td>
                        {canUpdate && nextStatus && (
                          <button
                            className={`btn btn-small ${nextStatus === 'COMPLETED' ? 'btn-success' : 'btn-primary'}`}
                            disabled={updatingStep === s.stepId}
                            onClick={() => handleUpdateStepStatus(s.stepId, nextStatus)}
                          >
                            {updatingStep === s.stepId
                              ? '...'
