...
import {
  ...
  getServiceRequestUpdates,
} from '../../services/api';
import { toast } from 'react-toastify';

const badge = (s) => <span className={`badge badge-${s.toLowerCase()}`}>{s.replace('_', ' ')}</span>;

// Determines the next allowed status(es) for a service request based on its
// current status. Officers move ASSIGNED → IN_PROGRESS → RESOLVED.
...
export default function UpdateServiceRequestPage() {
    ...
    (async () => {
      try {
        const [r, u] = await Promise.all([
          ...
          getServiceRequestUpdates(requestId),
        ]);
        setReq(r.data);
        setUpdates(u.data);
        const next = allowedNext(r.data.status);
        if (next.length > 0) {
          setForm((f) => ({ ...f, status: next[0] }));
        }
      } catch {
  ...
  const allowed = allowedNext(req.status);

  return (
    <>
      <div className="card">
        <div className="card-title">
          <span className="icon icon-blue"></span>Update Request #{req.requestId}
        ...
        {allowed.length === 0 ? (
          <div className="info-tip" style={{ width: '100%' }}>
            <span className="tip-icon"></span>This request cannot be updated further.
          </div>
        ) : (
          <form onSubmit={handleSubmit}>
            <div className="info-tip" style={{ width: '100%', marginBottom: 20 }}>
              <span className="tip-icon"></span>Transition: <strong>{req.status.replace('_', ' ')}</strong> → <strong>{form.status.replace('_', ' ')}</strong>
            </div>
            ...
            <div className="form-group">
              <label>Notes / Update Details <span style={{ color: 'var(--red-600, #dc2626)' }}>*</span></label>
              <textarea
                value={form.notes}
            ...
            <div className="actions-row">
              ...
              <button type="button" className="btn btn-outline" onClick={() => navigate(-1)}>Cancel
              </button>
            </div>
          </form>
        )}
          ...
          <div className="timeline">
            {updates.map((u, i) => (
              <div className={`timeline-item ${i === 0 ? 'completed' : 'active'}`} key={u.updateId}>
                ...
                <div className="timeline-title">{badge(u.status)}{u.status !== 'ASSIGNED' && <> — by {u.officerName}</>}</div>
