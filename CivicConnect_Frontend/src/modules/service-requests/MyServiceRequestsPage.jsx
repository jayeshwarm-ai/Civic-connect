...
import { PageHeader, StatCard, FeedbackPromptModal, ConfirmModal } from '../../components/ui';
...
export default function MyServiceRequestsPage() {
  ...
  // Pending confirm dialog. null = none open.
  // Shape: { title, message, variant, confirmLabel, run: async () => void }
  ...
  return (
    <>
      ...
      {/* Confirm modal — used for both Close and Withdraw actions.
          The pending state carries the message + run() so one modal handles both. */}
      <ConfirmModal
        open={!!pending}
        title={pending?.title}
