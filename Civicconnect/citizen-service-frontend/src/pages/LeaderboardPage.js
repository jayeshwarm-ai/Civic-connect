...
export default function LeaderboardPage() {
  ...
  return (
    <>
      <div className="welcome-banner" style={{ background: 'linear-gradient(135deg, #0f172a 0%, #7c2d12 50%, #ea580c 100%)' }}>
        <h2>🏆 Officer Satisfaction Leaderboard</h2>
        <p>Ranked by average citizen feedback scores across all service requests.</p>
      </div>
        ...
        <div className="card" style={{ borderLeft: '4px solid var(--primary)' }}>
          ...
          <div className="stats-row">
            <div className="stat-card">
              ...
              <div><div className="stat-value">{myMetric.averageScore?.toFixed(1) || '—'}</div><div className="stat-label">Avg Score</div></div>
            </div>
            <div className="stat-card">
              <div className="stat-icon" style={{ background: '#dbeafe' }}>📊</div>
              <div><div className="stat-value">{myMetric.totalFeedbackCount}</div><div className="stat-label">Total Reviews</div></div>
            </div>
            <div className="stat-card">
              <div className="stat-icon" style={{ background: '#d1fae5' }}>🏆</div>
              <div>
                <div className="stat-value">
                  #{metrics.findIndex(m => m.officerUserId === user.userId) + 1 || '—'}
                </div>
