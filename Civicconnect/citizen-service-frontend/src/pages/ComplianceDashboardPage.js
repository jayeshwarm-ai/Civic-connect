...
import { getComplianceRecordsByResult, getAuditsByOfficer, getServiceRequestsByStatus, getComplianceRecordsByEntity } from '../services/api';
    ...
    (async () => {
      try {
        const [passRes, failRes, auditsRes, closedRes] = await Promise.all([
          ...
          getAuditsByOfficer(user.userId).catch(() => ({ data: [] })),
  ...
  return (
    <>
      <div className="welcome-banner" style={{ background: 'linear-gradient(135deg, #0f172a 0%, #1e3a5f 50%, #0e7490 100%)' }}>
        ...
        <p>Review compliance records, manage audits, and ensure regulatory standards are met.</p>
      </div>

      <div className="stats-row">
        <div className="stat-card">
          <div className="stat-icon" style={{ background: '#fef3c7' }}>⏳</div>
          <div><div className="stat-value">{pendingRequests.length}</div><div className="stat-label">Pending Review</div></div>
        </div>
        <div className="stat-card">
          <div className="stat-icon icon-blue" style={{ background: '#e0e7ff' }}>📋</div>
          <div><div className="stat-value">{allRecords.length}</div><div className="stat-label">Total Records</div></div>
        </div>
        <div className="stat-card">
          <div className="stat-icon icon-green" style={{ background: '#d1fae5' }}>✅</div>
          <div><div className="stat-value">{passRecords.length}</div><div className="stat-label">Passed</div></div>
        </div>
        <div className="stat-card">
          <div className="stat-icon icon-red" style={{ background: '#fee2e2' }}>❌</div>
          <div><div className="stat-value">{failRecords.length}</div><div className="stat-label">Failed</div></div>
        </div>
      </div>
      ...
      {tab === 'audits' && (
        <div className="card">
          <div className="card-title"><span className="icon icon-green">🔍</span> My Audits ({audits.length})</div>
          {audits.length === 0 ? (
            <div className="empty-state">
              <div className="empty-icon">🔍</div>
              <h3>No audits yet</h3>
              <p>Create an audit to review compliance for a scope of requests or resolutions.</p>
            </div>
          ) : (
            <div className="table-container">
              <table>
                <thead>
                  <tr>
                    <th>ID</th><th>Scope</th><th>Status</th><th>Findings</th><th>Created</th><th>Actions</th>
                  </tr>
                </thead>
                <tbody>
                  {audits.map(a => (
